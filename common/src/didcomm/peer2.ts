/**
 * DID Peer Method 2 Implementation
 * 
 * Implements did:peer:2 generation and resolution.
...
/**
 * DID Peer 2 utility class
 */
export default class DIDPeer {
  /**
   * Convert a key to multibase encoding
   */
  static keyToMultibase(key: Uint8Array, prefix: multicodec.CodecName): string {
    const buffer = multibase.encode('base58btc', multicodec.addPrefix(prefix, key));
  ...
  /**
   * Generate a did:peer:2 DID
   * 
   * @param signingKeys - Array of Ed25519 signing keys
   * @param encryptionKeys - Array of X25519 encryption keys
   * @param serviceBlock - Service endpoint configuration
   * @returns The generated did:peer:2 DID
   */
  static generate(
  ...
  /**
   * Transform old service style to new
   */
  ...
  /**
   * Expand common string abbreviations
   */
    ...
    const expanded = Object.fromEntries(
      Object.entries(input).map(([key, value]) => {
        ...
        if (typeof value == 'string') {
          ...
          expandedValue = value.map(
            (item: any) => reverseCommonStringAbbreviations[item] || item
          );
  ...
  /**
   * Resolve a did:peer:2 DID into a DID Document
   * 
   * @param did - The did:peer:2 DID to resolve
      ...
      switch (purposeCode) {
        ...
        case purposeCodeList['Service']: {
          ...
          services = services
            ...
            .filter((service: any) => {
              return service.type == 'DIDCommMessaging';
            });
