...
export default function AdminDashboard() {
  ...
  const [invite, setInvite] = useState<{ url: string; qr: string; challenge: string; fields?: string[] } | null>(null)
  ...
  async function createInvitation() {
    setError(null)
    try {
      const fields = requestedFields.split(',').map((s) => s.trim()).filter(Boolean)
      const resp = await api.createPresentationRequestInvitation({
        ...
        requestedFields: fields.length > 0 ? fields : undefined,
        ttl: 3600,
      })
    ...
    <div className="bank-container">
      ...
      <div className="bank-header">
        <div>
          ...
          <p>Decentralized Identity Credential Verification System - Admin Dashboard</p>
        </div>
      ...
      {activeTab === 'request' && (
        <div className="bank-card">
          ...
          <div style={{ marginTop: '1.5rem' }}>
            <label className="label">Requested Fields (comma-separated, optional)</label>
            <input
              className="bank-input"
              value={requestedFields}
              onChange={(e) => setRequestedFields(e.target.value)}
              placeholder="e.g. name, dateOfBirth, address (leave empty for all fields)"
            />
            <p style={{ color: '#718096', fontSize: '0.875rem', marginTop: '0.5rem' }}>
              💡 <strong>Selective Disclosure:</strong> Specify only the fields you need. The holder will only share these fields, protecting their privacy.
            </p>
          </div>
