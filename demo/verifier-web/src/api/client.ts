...
export const api = {
  ...
  createPresentationRequestInvitation: (body: { requestedCredentials: string[]; ttl?: number }) => fetch(`${base}/invitations/presentation-request`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then((r) => j<{ invitation: any; invitationUrl: string; qrCode: string; challenge: string }>(r)),
